...
def vgg_16(inputs,
    ...
    with tf.variable_scope(scope, 'vgg_16', [inputs]):
        ...
        net = slim.conv2d(net, num_classes, [1, 1],
                        ...
                        normalizer_fn=None,
                        scope='fc8')
        print(net)  # Tensor("vgg_16/fc8/BiasAdd:0", shape=(1, 1, 1, 1000), dtype=float32)
        # 由于用卷积的方式模拟全连接层，所以输出需要平铺
        if spatial_squeeze:
            # 返回一个张量，这个张量是将原始input中所有维度为1的那些维都删掉的结果。
            # axis可以用来指定要删掉的为1的维度，此处要注意指定的维度必须确保其是1，否则会报错
            # https://blog.csdn.net/duanlianvip/article/details/98183497
            net = tf.squeeze(net, [1, 2], name='fc8/squeezed')
            print(net)  # Tensor("vgg_16/fc8/squeezed:0", shape=(1, 1000), dtype=float32)
        return net